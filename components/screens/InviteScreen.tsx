...
const InviteScreen: React.FC<InviteScreenProps> = ({ user, listName, members, onInvite, onDeleteMember }) => {
                    ...
                    {members.map(member => (
                        <li key={member.id} className="p-4 flex justify-between items-center">
                            <div className="flex items-center gap-3">
                                <img src={member.avatar || `https://api.dicebear.com/8.x/initials/svg?seed=${member.name}`} alt={member.name} className="w-10 h-10 rounded-full" />
                                <div>
                            ...
                            {member.id !== user?.id && (
                                <button onClick={() => onDeleteMember(member.id)} className="p-1 text-red-500 hover:text-red-700" aria-label={`Remover ${member.name}`}>
                                    <TrashIcon />
                                </button>
                            )}
                        </li>
