// components/screens/InviteScreen.tsx
...
const InviteScreen: React.FC = () => {
  ...
  const { name: listName, members } = activeList;
  ...
  const handleInviteMember = (email: string) => {
    ...
    updateActiveList({ members: [...members, newMember] });
            ...
            {members.map(member => (
              <li key={member.id} className="p-4 flex justify-between items-center">
                ...
                {member.id !== '1' && ( // Don't allow deleting the main user
                  <button onClick={() => handleDeleteMember(member.id)} className="p-1 text-red-500 hover:text-red-700">
                    <TrashIcon />
                  </button>
                )}
              </li>
