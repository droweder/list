...
const ItemFormScreen: React.FC<ItemFormScreenProps> = ({ item, categories, onSave, onDelete, onCancel }) => {
  ...
  return (
    <div>
        <Header title={item ? 'Editar Item' : 'Adicionar Item'} />
            ...
            <form onSubmit={handleSubmit} className="space-y-6 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md">
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between gap-4">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-center gap-4">
                        {item && (
                            <button type="button" onClick={() => onDelete(item.id)} className="w-full sm:w-auto px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-red-700 bg-red-100 hover:bg-red-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500">
                                Excluir Item
                            </button>
                        )}
