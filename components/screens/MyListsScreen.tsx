...
const MyListsScreen: React.FC<MyListsScreenProps> = ({ lists, activeListId, onSelectList, onAddList, onDeleteList, onRenameList }) => {
            ...
            <div className="max-w-4xl mx-auto p-4 space-y-6">
                ...
                <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md">
                    <ul className="divide-y divide-gray-200 dark:divide-gray-700">
                        {lists.length > 0 ? (
                            lists.map(list => (
                                <li key={list.id} className={`p-4 flex flex-col sm:flex-row justify-between sm:items-center gap-4 ${list.id === activeListId ? 'bg-primary-50 dark:bg-primary-900/20' : ''}`}>
                                    {editingListId === list.id ? (
                                        ...
                                        <div onClick={() => onSelectList(list.id)} className="flex-grow cursor-pointer">
