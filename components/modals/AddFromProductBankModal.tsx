  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center px-4">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-2xl shadow-xl w-full max-w-lg max-h-[80vh] flex flex-col">
          ...
          {Object.keys(filteredProducts).sort().map(category => (
            <div key={category}>
              ...
              {filteredProducts[category].map(product => {
                const isAdded = listItems.some(item => item.name.toLowerCase() === product.name.toLowerCase());
                return (
                  <div key={product.id} className="flex justify-between items-center py-2 px-1 border-b border-gray-200 dark:border-gray-700">
                    ...
                    <button
                      ...
                      className={`w-28 text-center py-1 rounded-md text-sm font-semibold transition-all duration-200 ${
                        isAdded ? 'bg-gray-200 dark:bg-gray-600 text-gray-500'
                                : addedProductId === product.id ? 'bg-green-500 text-white'
                                : 'bg-primary-600 hover:bg-primary-700 text-white'
                      }`}
                    >
                      {isAdded ? 'Adicionado' : addedProductId === product.id ? 'Adicionado!' : 'Adicionar'}
                    </button>
                  </div>
                );
              })}
        ...
        {Object.keys(filteredProducts).length === 0 && !showNewProductForm && (
          <div className="text-center my-4">
            <p className="text-gray-500 dark:text-gray-400 mb-2">"{searchTerm}" não encontrado.</p>
            <button onClick={() => setShowNewProductForm(true)} className="text-primary-600 hover:underline font-semibold">
        ...
        {showNewProductForm && (
          <div className="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700">
            <h3 className="text-lg font-semibold text-gray-800 dark:text-white">Novo Produto</h3>
